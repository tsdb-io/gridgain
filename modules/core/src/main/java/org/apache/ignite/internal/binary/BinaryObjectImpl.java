...
public final class BinaryObjectImpl extends BinaryObjectExImpl implements Externalizable, KeyCacheObject {
    ...
    @Override public boolean putValue(final ByteBuffer buf, int off, int len) throws IgniteCheckedException {
        ...
        return CacheObjectAdapter.putValue(type, buf, off, len, arr, start);
    }
    ...
    /** */
    private void tryCompress(CompressionSpi compressionSpi) {
        ...
        byte[] tryCompressed = compressionSpi.tryCompress(arr);
    ...
    @Override public KeyCacheObject prepareForCache(CacheObjectContext ctx, boolean compress) {
        if (!detached())
            ...
            tryCompress(ctx.compressionSpi());
        else
            compressedArr = UNCOMPRESSED;
